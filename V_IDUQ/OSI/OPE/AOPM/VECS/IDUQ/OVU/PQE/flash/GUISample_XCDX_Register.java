...
public class GUISample_XCDX_Register {
	public void Register(GUISample gUISample) {
		gUISample.load.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					int confirm= jOptionPane.showConfirmDialog(gUISample.canvas, StableAnnotation.ATTENSION_LOAD_ENSURE);
					if(0!= confirm) {
						gUISample.rightBotJTextPane.setText(StableAnnotation.ATTENSION_CANCELLED_OPERATION);
					...
					gUISample.fileCurrentpath= filedialog.getDirectory()+ filedialog.getFile();
					System.out.println(gUISample.fileCurrentpath);
					if(null== gUISample.fileCurrentpath|| gUISample.fileCurrentpath.isEmpty()|| !gUISample.fileCurrentpath.contains
					...
					while(needDeleteNode!= null) {
						gUISample.first.first= gUISample.first.deletNode(gUISample.first.first, needDeleteNode.name, needDeleteNode.ID
					...
					gUISample.first.first= LoadFile.Load(gUISample.first.first, gUISample.nodeView, file, gUISample.first);
		...
		gUISample.save.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				javax.swing.JOptionPane jOptionPane= new JOptionPane(StableAnnotation.ATTENSION_UPDATE_ENSURE
						+ gUISample.fileCurrentpath + StableAnnotation.MARK_QUESTION);
				int confirm= jOptionPane.showConfirmDialog(gUISample.canvas, StableAnnotation.ATTENSION_UPDATE_ENSURE
						+ gUISample.fileCurrentpath + StableAnnotation.MARK_QUESTION);
				if(0!= confirm) {
					gUISample.rightBotJTextPane.setText(StableAnnotation.ATTENSION_CANCELLED_OPERATION);
					gUISample.rightBotJTextPane.validate();
			...
			public void actionPerformed(ActionEvent e) {
				...
				Here:
					for(int i= 0; i< files.length; i++) {
						...
						if(gUISample.nodeReflection.containsKey(OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSI.SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS)) {
							gUISample.rightBotJTextPane.setText("请勿重复添加，节点已存在"+ System.currentTimeMillis());
							continue Here;
							//return;
						}
						gUISample.nodeReflection.put(OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSI.SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS, null);
						OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSI.register(gUISample.tableData_old, gUISample.text, gUISample.u, gUISample._A, gUISample.pos);
						try {
							...
							if(OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSI.SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS.contains("添加")
								...
								gUISample.u.searchList.add(OSU_AVQ_ASQ_ASQ_OCQ_OSI_PCI_PCU_MCI_MCU_MSI); //memory increment
							}
							//searchList
							//
						...
						gUISample.nodeView.first= gUISample.nodeView.link.addNode(gUISample.nodeView.first
		...
		gUISample.boot.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					BootNeroCell.bootCell(gUISample.first.first, gUISample.rightBotJTextPane, gUISample.canvas);
		...
		gUISample.delete.addActionListener(new java.awt.event.ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					int confirm= jOptionPane.showConfirmDialog(gUISample.canvas, StableAnnotation.ATTENSION_CANCEL_ENSURE);
					if(0!= confirm) {
						gUISample.rightBotJTextPane.setText(StableAnnotation.ATTENSION_CANCELLED_OPERATION);
						gUISample.rightBotJTextPane.validate();
						return;
					...
					while(node!= null) {
						gUISample.first.first= gUISample.first.deletNode(gUISample.first.first, node.name, node.ID, node.primaryKey);
						if(null== node.next) {
							break;
						}
						node= node.next;
				...
				gUISample.rightBotJTextPane.setText(StableAnnotation.ATTENSION_DELETE);
				gUISample.rightBotJTextPane.validate();
			}	
		});
		gUISample.leftSplitPane.addPropertyChangeListener(new java.beans.PropertyChangeListener() {  
			public void propertyChange(java.beans.PropertyChangeEvent evt) {  
				if (evt.getPropertyName().equals(JSplitPane.DIVIDER_LOCATION_PROPERTY)) {  
					...
					gUISample.nodeProject.height= gUISample.leftSplitPane.getDividerLocation();
					gUISample.nodeProject.setBounds(0, 0, gUISample.leftSplitPane.getWidth(), gUISample.leftSplitPane
							.getDividerLocation());
		...
		gUISample.mainSplitPane.addPropertyChangeListener(new java.beans.PropertyChangeListener() {  
			public void propertyChange(java.beans.PropertyChangeEvent evt) {  
				if (evt.getPropertyName().equals(JSplitPane.DIVIDER_LOCATION_PROPERTY)) {  
					//action code 
					gUISample.nodeProject.width= gUISample.mainSplitPane.getDividerLocation();
					gUISample.nodeProject.height= gUISample.leftSplitPane.getDividerLocation();
					gUISample.nodeProject.setBounds(0, 0, gUISample.mainSplitPane.getDividerLocation()
		...
		gUISample.nodeView.tree.addTreeSelectionListener(new TreeSelectionListener() {
			public void valueChanged(TreeSelectionEvent evt) {
				DefaultMutableTreeNode note= (DefaultMutableTreeNode) gUISample.nodeView.tree.getLastSelectedPathComponent();
				String tr= null;
				if(note!= null){
				...
				if(tr!=null){
					...
					gUISample.rightBotJTextPane.setText("节点名："+ gUISample.treeNodeName);
					gUISample.rightBotJTextPane.validate();
				}
			}
		});
		gUISample.menuItem.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(gUISample.treeNodeName!=null){
					try {
						gUISample.first.first= gUISample.first.addNode(gUISample.first.first, gUISample.treeNodeName, 100, 50, gUISample.nodeView.first);
						gUISample.righttopScrollPane.validate();
					} catch (CloneNotSupportedException e1) {
						gUISample.rightBotJTextPane.setText(StableAnnotation.NODE_ADD_ERROR);
						gUISample.rightBotJTextPane.validate();
					} catch (InstantiationException e1) {
						gUISample.rightBotJTextPane.setText(StableAnnotation.NODE_ADD_ERROR);
						gUISample.rightBotJTextPane.validate();
			...
			public void actionPerformed(ActionEvent e) {
				...
				while(node!= null){
					if(node.name.equals(gUISample.canvas.currentNodeName)&&node.ID== gUISample.canvas.currentNodeID
						...
						try {
							node.thisFace.MEI_MSU(gUISample.rightBotJTextPane, gUISample.canvas);
							...
							new OSI.OPE.SI.MCI.OEI.OVU.PQE.extOSGI.OSGI_chansfer(node, gUISample.first.first);
						} catch (IOException e1){
							gUISample.rightBotJTextPane.setText(StableAnnotation.NODE_UPDATE_ERROR);
							gUISample.rightBotJTextPane.validate();
						} 
					}
					node= node.next;
				}	
				gUISample.rightBotJTextPane.setText(StableAnnotation.NODE_UPDATE_SUCCESS);
				gUISample.rightBotJTextPane.validate();
			}
		}); 
		...
		gUISample.show.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					if(node.name.equals(gUISample.canvas.currentNodeName)&&node.ID== gUISample.canvas.currentNodeID
						...
						if(!node.thisFace.showed){
							try {
								...
								//e1.printStackTrace();
								gUISample.rightBotJTextPane.setText(StableAnnotation.NODE_INSPECT_ERROR);
								gUISample.rightBotJTextPane.validate();
							}  
						}else{
							node.thisFace.SQ_OSU_MSQ_OSU_AVQ_ASQ_AVQ_ASQ_OVQ_OSQ_VSQ.setVisible(true);  
				...
				gUISample.rightBotJTextPane.setText(StableAnnotation.NODE_INDICATE_SUCCESS);
				gUISample.rightBotJTextPane.validate();
			}
		}); 
		gUISample.dNode.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				while(node!= null){
					if(node.name.equals(gUISample.canvas.currentNodeName)&&node.ID== gUISample.canvas.currentNodeID
							&& node.primaryKey.equalsIgnoreCase(gUISample.canvas.currentNodePrimaryKey)){
						gUISample.first.first= gUISample.first.deletNode(gUISample.first.first, node.name, node.ID, node.primaryKey);
						gUISample.updateRelatedLine.doUpdateRelatedLine(gUISample.first.first, gUISample.canvas.currentNodeName, gUISample.canvas.currentNodeID
		...
		gUISample.dLine.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				node=gUISample.first.first;
				while(node!=null){
					if(node.beconnect&&node.name.equals(gUISample.canvas.currentNodeName)&& node.ID==gUISample.canvas.currentNodeID
							&& node.primaryKey.equals(gUISample.canvas.currentNodePrimaryKey)){

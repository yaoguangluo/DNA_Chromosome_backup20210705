...
public class GUISample_XCDX_CreateMap {	
	public void CreatMap(GUISample gUISample) throws IOException {
		...
		gUISample.nodeView= new NodeShow(gUISample, gUISample.tableData_old, gUISample.text, gUISample.popupMenu, gUISample.u, gUISample._A, gUISample.pos);
		...
		gUISample.canvas= new ThisCanvas(gUISample.threadApplet, gUISample.first, gUISample.nodeView, gUISample.nodeMenu, gUISample.rightBotJTextPane);
...
//						}
		...
		execButton6.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				@SuppressWarnings("static-access")
				int confirm= jOptionPane.showConfirmDialog(gUISample.canvas, "确认卸载吗？");
				if(0!= confirm) {
					gUISample.rightBotJTextPane.setText(StableAnnotation.ATTENSION_CANCELLED_OPERATION);
					gUISample.rightBotJTextPane.validate();
					return;
				...
				while(linkNode!= null) {//benshen
					if(gUISample.nodeReflection.containsKey(linkNode.SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS)) {
						gUISample.nodeReflection.remove(linkNode.SQ_OSU_MSQ_OSU_AVQ_ASQ_SQ_VPC_PCS);

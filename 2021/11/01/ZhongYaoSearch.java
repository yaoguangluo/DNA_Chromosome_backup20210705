...
public class ZhongYaoSearch{
	public static void zhongYaoSearch(App app, String zhongyao, String forE, String key) {
		...
		while(iteratorForCopy.hasNext()) {
			...
			if(app.appConfig.SectionJPanel.jlabel_peizhi_di2233.isSelected()) {//文学性测试
				tempsPCA= 文学性.文言文全文处理(temps);
				temps= 文学性.文言文线性处理(app, tempsPCA);
			}
			...
			Here:
				while(iteratorWordFrequency.hasNext()) {  
					...
					= mapSearchWithoutSort.get(mapSearchaAtII);
...
//因为这几行是新电脑开发的只有3行数据测试,老电脑写的拼音索引没用到,就出现这个问题
						...
						if(app.pos.get(mapSearchaAtII).contains("名")
								|| app.pos.get(mapSearchaAtII).contains("动")
								|| app.pos.get(mapSearchaAtII).contains("形")
								|| app.pos.get(mapSearchaAtII).contains("谓")) {
							reg[copyCount]+= 2;
							if(tempsPCA.contains(mapSearchaAtII)) {
								reg[copyCount]<<= 1;
							}
						...
						score_code[copyCount]+= (iteratorForCopyString.contains(mapSearchaAtII)
								? 2: 1) 
								<< (!app.pos.get(mapSearchaAtII).contains("名") //测试了下不错，误差小，罗瑶光202111011900
										? app.pos.get(mapSearchaAtII).contains("动")? 3: 0: 7) //用位移替换10、1、150
...
//										? app.pos.get(mapSearchaAtII).contains("动")? 10: 1: 150) //用位移替换10、1、150
			...
			//
			if(score[copyCount].contains("紫雪散")) {
			//	int j=0;
			}
			int code= 200;
			int tempb= 0;
			...
			for(int i = app.copy.size()-1; i > -1; i--) {
				...
				tableData[new_count]= new Object[]{
						(app.dic_index.get(score[i])== null? ""
								: app.dic_index.get(score[i])).toString().replaceAll("\\s*", ""), 
						score_code[i], score[i],
						(app.dic_yw.get(score[i])== null? ""
								: app.dic_yw.get(score[i])).toString().replaceAll("\\s*", ""), 
						...
						(app.dic_yl.get(score[i])==null?"详情参考相关书籍"
								:app.dic_yl.get(score[i])).toString().replaceAll("\\s*", ""),
						(app.dic_xw.get(score[i])== null? ""
								: app.dic_xw.get(score[i])).toString().replaceAll("\\s*", ""), 
						(app.dic_jm.get(score[i])== null? ""
								: app.dic_jm.get(score[i])).toString().replaceAll("\\s*", ""), 	
						(app.dic_xz.get(score[i])==null?""
								:app.dic_xz.get(score[i])).toString().replaceAll("\\s*", ""), 
						(app.dic_jj.get(score[i])==null?""
								:app.dic_jj.get(score[i])).toString().replaceAll("\\s*", ""), 
						(app.dic_cy.get(score[i])==null?""
								:app.dic_cy.get(score[i])).toString().replaceAll("\\s*", ""),
						(app.dic_ya.get(score[i])==null?""
								:app.dic_ya.get(score[i])).toString().replaceAll("\\s*", ""), 
						(app.dic_zf.get(score[i])==null?""
								:app.dic_zf.get(score[i])).toString().replaceAll("\\s*", ""),
						(app.dic_cj.get(score[i])==null?""
								:app.dic_cj.get(score[i])).toString().replaceAll("\\s*", "")};

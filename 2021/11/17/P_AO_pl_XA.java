	...
	public static void P_AggregationLimitMap(String[] sets
		...
		if(sets[1].equalsIgnoreCase("进行字符排序")) {
			...
			while(iterators.hasNext()) {
				...
				Map<String, Object> rowValue= (Map<String, Object>)map.get(StableShellETL.SHELL_ETL_ROWVALUE);
...
//							, SortStringDemo.biHua, 7, 70);
		...
		if(sets[1].equalsIgnoreCase("颜色标记为")) {
			...
			while(iterator.hasNext()) {
				...
				Map<String, Object> rowMap = new HashMap<>();
				if(sets[1].equalsIgnoreCase("颜色标记为")) {
					Map<String, Object> map= (Map<String, Object>)row.get(StableShellETL.SHELL_ETL_ROWVALUE);
					...
					row.put(StableShellETL.SHELL_ETL_ROWVALUE, map);
					outputTemp.add(row);
				}
			}
			output.clear();
		...
		//之后分出去准备思考怎么分类。罗瑶光
		if(String_ESU.stringArrayToString(sets).contains("替换")) {
			Iterator<Map<String, Object>> iterator= output.iterator();
			int count= 0;
			while(iterator.hasNext()) {
				int rowid= count++;

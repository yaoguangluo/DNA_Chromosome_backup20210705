...
//因为这几行是新电脑开发的只有3行数据测试,老电脑写的拼音索引没用到,就出现这个问题,我先注释掉,以后优化. 罗瑶光20210516
						...
						score_code[copyCount]+= (iteratorForCopyString.contains(mapSearchaAtII)? 2: 1) 
								* (!app.pos.get(mapSearchaAtII).contains("名")? app.pos.get(mapSearchaAtII).contains("动")? 10: 1: 150) //动、、名
		...
		Here:
			for(int i = app.copy.size()-1; i > -1; i--) {
				...
				//20210724 //合并后数据冗余 eclipse不上报错误， 因为空格字符null问题， 害我调试了半天。bin的debug函数被本地欺骗了。
				//催化比值rot dna 催化计算
				if(!app.nameFeelFilter.getText().isEmpty()) {
					String wei= !app.dic_xw.containsKey(score[i])? "": app.dic_xw.get(score[i]).toString().replaceAll("\\s*", "");
				...
				if(null!= app.name_filter_not_have.getText()) {
					if(!app.name_filter_not_have.getText().replace(" ", "").isEmpty()) {
						String wei= !app.dic_jm.containsKey(score[i])? "": app.dic_jm.get(score[i]).toString().replaceAll("\\s*", "");

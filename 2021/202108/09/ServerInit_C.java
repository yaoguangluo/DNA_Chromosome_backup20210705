...
//		}
	...
	public static void IV_Service(JTextPane jTextPane, String 前端接口Txt) throws IOException {
		try {
	...
	public static void IV_Server(App app) throws IOException {
		...
		IV_Service(app.appConfig.SectionJPanel.jTextPane, app.数据库接口Txt);
		time_P.end();
		if(app.appConfig.SectionJPanel.jTextPane!= null) {
			String text= app.appConfig.SectionJPanel.jTextPane.getText();
		...
		while(true){
			if(sleeper_H.getThreadsCount()< StableWeb.SLEEPERS_RANGE){
				Sleeper sleeper= new Sleeper(app, StableHTTP.PORT_DATABASE);
				try {
